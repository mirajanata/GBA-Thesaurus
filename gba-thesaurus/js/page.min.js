var page = {
    BASE: location.protocol + "//" + location.host + location.pathname, urlParams: new URLSearchParams(window.location.search), init: function () {
        var a = (navigator.language || navigator.language).substring(0, 2); $("#appsCard").toggle(); this.urlParams.has("lang") && (a = this.urlParams.get("lang")); "de" !== a ? (a = "en", $("#lang").text("EN")) : $("#lang").text("DE"); lang.load(a); this.setNavbarFooter(); search.insertSearchCard(); var b = this.urlParams; if (b.has("search")) search.insertSearch(decodeURI(b.get("search"))), this.insertProjCards();
            ...
            (this.insertPageDesc(), this.insertComments("proj_desc", lang.LIST_THESAURUS_PROJECTS), this.insertComments("other_desc", [lang.DESC_INSPIRE, lang.DESC_LINKEDDATA]), this.insertProjCards()); search.initProjects(); document.documentElement.setAttribute("lang", a); this.updateSharingUrl($("#fbShare")); this.updateSharingUrl($("#twShare")); this.updateSharingUrl($("#liShare"))
    }, updateSharingUrl: function (a) {
        var b = encodeURIComponent(window.location.href); b = a.attr("href").replace("wwwgeolbanet", b).replace("wwwgeolbanet",
            b); a.attr("href", b)
    }, updateSharingTexts: function (a) { this.updateSharingText($("#fbShare"), a); this.updateSharingText($("#twShare"), a); this.updateSharingText($("#liShare"), a) }, updateSharingText: function (a, b) { var c = encodeURIComponent(b); c = a.attr("href").replace("GBA%20Thesaurus", c).replace("GBA%20Thesaurus", c); a.attr("href", c) }, setLang: function (a) {
    -1 != location.href.indexOf("lang=") ? "de" == a ? location.replace(location.href.replace("lang=en", "lang=de")) : location.replace(location.href.replace("lang=de", "lang=en")) :
        -1 != location.href.indexOf("?") ? location.replace(location.href + "&lang=" + a) : location.replace(location.href + "?lang=" + a)
    }, openParaLink: function (a) { window.open(this.BASE + "?" + a + "&lang=" + lang.ID, "_self", "", "false") }, toggleRead: function (a, b, c) { c = $("#" + b).is(":visible") ? "&#9654; <em>" + c + " ..</em>" : "&#9660; <em>" + c + " ..</em>"; $("#" + a).html(c); $("#" + b).slideToggle() }, openFeedBack: function () {
        var a = "Anfrage", b = ""; 0 < $("#uri").length && (b = "URI: " + $("#uri").text()); 0 < $(".altLabel").length && (a = $(".altLabel").html().replace(/<span class="lang">/g,
            "[").replace(/<\/span>/g, "] ").replace(/<li>/g, "").replace(/<\/li>/g, "").replace(/  /g, "")); window.location.href = "mailto:thesaurus@geologie.ac.at?subject=" + a + "&body=" + b
    }, insertSideCard_projectInfo: function (a) { $("#proj_links").append('<div class="card border-info mb-3">\n                                <h4 class="card-header">' + a.name + " (" + lang.TOPIC + ')</h4>\n                                <div id="' + a.id + 'Card" class="card-body">' + a.desc + "</div>\n                            </div>") }, insertInfo: function (a) {
        $("#page_desc").empty().append("<br>" +
            lang["DESC_" + a.toUpperCase()])
    }, setNavbarFooter: function () { $("#LABEL_CONTACT").html(lang.LABEL_CONTACT); $("#contact").html(lang.LABEL_CONTACT); $("#license").html(lang.LABEL_LICENSE); $("#disclaimer").html(lang.LABEL_DISCLAIMER); $("#IMG_GBALOGO").attr("src", "img/" + lang.IMG_GBALOGO) }, insertProjCards: function () {
